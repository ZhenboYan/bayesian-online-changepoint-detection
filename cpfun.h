...
std::vector<float> select_row_col(const std::vector<std::vector<float>> &matrix, int ls, int le, int r){
    std::vector<float> v;
    for (int i=ls;i<=le;++i)
...
static std::vector<float> muT = {0}; // the past changes
static std::vector<float> betaT  = {1};// the past changes
